...
export default function ImageWithSkeleton({ src, alt, className }: { src: string; alt: string; className?: string }){
  ...
  return (
    <div className={className} style={{position:'relative'}}>
      ...
      <img src={src} alt={alt} className={`w-full h-full object-cover rounded-xl ${loaded? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`} onLoad={()=>setLoaded(true)} />
    </div>
  );
}
